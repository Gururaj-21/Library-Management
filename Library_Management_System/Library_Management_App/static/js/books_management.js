document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadBooks() {
        ...
        for (const book of books) {
            let borrowerName = 'N/A'; // Default value
            if (book.id) {
                // Fetch borrower information if `borrower_id` is available
                const borrowerResponse = await fetch(`/borrower_name/${book.id}/`);
                if (borrowerResponse.ok) {
                    const borrower = await borrowerResponse.json();
                    borrowerName = borrower.name;
                    borrowerDate = borrower.borrowed_date;
                }
            }
